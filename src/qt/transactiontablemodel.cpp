...
static int column_alignments[] = {
    ...
    Qt::AlignLeft | Qt::AlignVCenter, /* blob */
...
{
    columns << QString() << QString() << tr("Date") << tr("Type") << tr("Address") << BitcoinUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit()) << tr("Info");
    priv->refreshWallet();
...
QString TransactionTableModel::formatTxBlob(const TransactionRecord* wtx) const
{
    return QString::fromStdString(wtx->blob);
}
...
QString TransactionTableModel::formatTooltip(const TransactionRecord* rec) const
{
    ...
    if (rec->type == TransactionRecord::RecvFromOther || rec->type == TransactionRecord::SendToOther ||
        ...
        tooltip += QString(" ") + formatTxBlob(rec);
    }
    return tooltip;
}
...
std::string TransactionTableModel::blob(int row, const QModelIndex& parent) const
{
    Q_UNUSED(parent);
    TransactionRecord* data = priv->index(row);
    return data->blob;
