...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::Keyco, nValue);
...
std::string TxToString(uint256 BlockHash, const CTransaction& tx)
{
    ...
    std::string OutputsContentCells[] = {_("#"), _("Redeemed in"), _("Address"), _("Amount"), _("Info")};
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (keyco.conf).");
            QMessageBox::warning(this, "Keyco Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
