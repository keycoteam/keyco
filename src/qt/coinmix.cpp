...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::Keyco)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeKeycoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeKeycoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeKeycoAmount = strAnonymizeKeycoAmount.remove(strAnonymizeKeycoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeKeycoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeKeycoAmount * COIN) nMaxToAnonymize = nAnonymizeKeycoAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeKeycoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeKeycoAmount));
        strAnonymizeKeycoAmount = strAnonymizeKeycoAmount.remove(strAnonymizeKeycoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeKeycoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
