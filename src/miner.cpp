...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        pblock->vtx[0].vout[0].blob = "test blob";
        if (!fProofOfStake) {
            pblock->vtx[0] = txNew;
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("KeycoMiner(): Signing new block failed \n");
                continue;
            }
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadKeycoMiner() exception");
    } catch (...) {
        LogPrintf("ThreadKeycoMiner() exception");
    }

    LogPrintf("ThreadKeycoMiner exiting\n");
}
