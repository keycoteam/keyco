...
const string strMessageMagic = "Keyco Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) return 360001 * COIN;
		
	int64_t nSubsidy;
	
	if(nHeight <= 102 && nHeight > 0) {
        nSubsidy = 1 * COIN;
	} else if (nHeight > 302 && nHeight <= 21703) {
		nSubsidy = 12 * COIN;
	} else if (nHeight > 21703 && nHeight <= 50504) {
		nSubsidy = 10 * COIN;
	} else if (nHeight > 50504 && nHeight <= 86505) {
		nSubsidy = 8 * COIN;
	} else if (nHeight > 86505 && nHeight <= 129706) {
		nSubsidy = 7 * COIN;
	} else if (nHeight > 129706 && nHeight <= 180107) {
		nSubsidy = 6 * COIN;
	} else if (nHeight > 180107) {
		nSubsidy = 5 * COIN;
	} 
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	int64_t ret = 0;
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[LIBRA] Adjusting Libra at height %d with %d masternodes (%d % locked Keyco) and %d Keyco supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (pindex->nHeight <= Params().LAST_POW_BLOCK() && block.IsProofOfStake())
        return state.DoS(100, error("ConnectBlock() : PoS disabled"),
            REJECT_INVALID, "PoS-disabled");
        ...
        try {
            ...
            if (false) // && block.GetHash() != hashGenesisBlock)
            {
               ...
               // creating a different genesis block:
                ...
                while(true)
                {
                    //printf("try to calc hash\n");
                    //scrypt_1024_1_1_256_sp(BEGIN(block.nVersion), BEGIN(thash), scratchpad);
                    //HashQuark(BEGIN(block.nVersion), BEGIN(thash));
                    thash = block.GetHash();
