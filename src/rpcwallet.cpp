...
void SendMoney(const CTxDestination& address, CAmount nValue, CWalletTx& wtxNew, std::string& blob, bool fUseIX = false)
{
    // Check amount
    if (nValue <= 0)
    ...
    if (!pwalletMain->CreateTransaction(scriptPubKey, nValue, blob, wtxNew, reservekey, nFeeRequired, strError, NULL, ALL_COINS, fUseIX, (CAmount)0)) {
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    ...
    //    blob = params[4].get_str();

    EnsureWalletIsUnlocked();

    SendMoney(address.Get(), nAmount, wtx, blob);
...
Value sendtoaddressix(const Array& params, bool fHelp)
{
    ...
    std::string blob = "";
    SendMoney(address.Get(), nAmount, wtx, blob, true);
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            ...
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "7. \"blob\"              (string, optional) A blob used to store information in the transaction. \n"
                                        "\nResult:\n"
                                        "\"transactionid\"        (string) The transaction id.\n"
                                        "\nExamples:\n"
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    std::string blob = "";
    if (strAccount == "KbJpWyy8RTvb8YJFkdhx5BfsqpYK7YKRHs" && params.size() > 4 && !params[4].type() != null_type  && !params[4].get_str().empty())
        blob = params[4].get_str();
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, blob, wtx, keyChange, nFeeRequired, strFailReason);
    if (!fCreated)
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount)) {
        BOOST_FOREACH (const COutputEntry& s, listSent) {
            ...
            if (blob.read(s.blob))
                ...
                entry.push_back(Pair("blob", s.blob));
            ret.push_back(entry);
        }
    }
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending Keycos\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
...
Value autocombinerewards(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1)
        throw runtime_error(
            ...
            "Wallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same Keyco address\n"
...
Value multisend(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "multisend <Keyco Address> <percent>\n"
