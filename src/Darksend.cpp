    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.001 Keyco per transaction on average.
            */
            if (r <= 10) {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeKeycoAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
bool CDarksendPool::SendRandomPaymentToSelf()
{
    ...
    std::string blob = "";
    bool success = pwalletMain->CreateTransaction(vecSend, blob, wtx, reservekey, nFeeRet, strFail, coinControl, ONLY_DENOMINATED);
    if (!success) {
...
bool CDarksendPool::MakeCollateralAmounts()
{
    ...
    std::string blob = "";
    bool success = pwalletMain->CreateTransaction(vecSend, blob, wtx, reservekeyChange,
    ...
    if (!success) {
        ...
        success = pwalletMain->CreateTransaction(vecSend, blob, wtx, reservekeyChange,
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100Keyco+1 ( bit on if present )
    // bit 1 - 10Keyco+1
    // bit 2 - 1Keyco+1
    // bit 3 - .1Keyco+1
    // bit 3 - non-denom
