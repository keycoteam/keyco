    ...
    const isminefilter& filter) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); ++i) {
        ...
        COutputEntry output = {address, txout.nValue, (int)i, txout.blob};
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 Keyco
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (int64_t v, DarKsendDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 Keyco over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputDarksendRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 Keyco+1 ( bit on if present )
            // bit 1 - 1000 Keyco+1
            // bit 2 - 100 Keyco+1
            // bit 3 - 10 Keyco+1
            // bit 4 - 1 Keyco+1
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    CCoinControl* coinControl = NULL;
    std::string blob = "";
    bool success = CreateTransaction(vecSend, blob, tx, reservekey, nFeeRet, strFail, coinControl, ALL_COINS, useIX, (CAmount)0);
    if (!success) {
...
bool CWallet::CreateTransaction(const vector<pair<CScript, CAmount> >& vecSend, std::string& blob,
    CWalletTx& wtxNew,
            ...
            while (true) {
                    ...
                    BOOST_FOREACH (const PAIRTYPE(CScript, CAmount) & s, vecSend) {
                        for (int i = 0; i < nSplitBlock; i++) {
                            if (i == nSplitBlock - 1) {
                                ...
                                txNew.vout.push_back(CTxOut((s.second / nSplitBlock) + nRemainder, s.first, blob));
                            } else
                                txNew.vout.push_back(CTxOut(s.second / nSplitBlock, s.first, blob));
                        }
                    }
                }
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 Keyco.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Darksend non-denominated funds for this transaction that are not equal 10000 Keyco.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    CTxOut newTxOut(nChange, scriptChange, blob);
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, std::string& blob, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, CAmount nFeePay)
{
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    std::string blog = "";
    return CreateTransaction(vecSend, blob, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, coin_type, useIX, nFeePay);
}
...
void CWallet::AutoCombineDust()
{
    ...
    for (map<CBitcoinAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
       ...
       MilliSleep(5000);
        ...
        CWalletTx wtxdummy;
        std::string blob = "";
        CreateTransaction(vecSend, blob, wtxdummy, keyChange, nFeeRet, strErr, coinControl, ALL_COINS, false, CAmount(0));
        vecSend[0].second = nTotalRewardsValue - nFeeRet - 500;

        if (!CreateTransaction(vecSend, blob, wtx, keyChange, nFeeRet, strErr, coinControl, ALL_COINS, false, CAmount(0))) {
...
bool CWallet::MultiSend()
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        string strErr;
        std::string blob = "";
        CreateTransaction(vecSend, blob, wtxdummy, keyChange, nFeeRet, strErr, cControl, ALL_COINS, false, CAmount(0));
        ...
        // Create the transaction and commit it to the network
        blob = "";
        if (!CreateTransaction(vecSend, blob, wtx, keyChange, nFeeRet, strErr, cControl, ALL_COINS, false, CAmount(0))) {
